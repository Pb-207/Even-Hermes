...
export function footerHint(_state: State): string {
  return ''; // 统一不显示底部操作提示
}
...
// 渲染会话历史(折叠+总字符预算)。无历史时返回 fallback。
function historyText(h: HermesMessage[] | undefined, fallback: string): string {
  if (!h || !h.length) return fallback;
  let lastAsst = -1, lastUser = -1;
  h.forEach((m, i) => { if (m.role === 'assistant') lastAsst = i; else lastUser = i; });
  const MAX_FULL = 200;  // 最新一轮对话单条完整上限
  const MAX_TOTAL = 500; // 总字符预算(≈2 屏);最新完整优先,其余折叠
  const fullSet = new Set<number>([lastAsst, lastUser].filter((x) => x >= 0));
  const newestFirst: string[] = [];
  let budget = MAX_TOTAL;
  for (let i = h.length - 1; i >= 0; i--) {
    const m = h[i];
    const full = fullSet.has(i);
    const text = full ? truncate(m.text, MAX_FULL) : truncate(m.text, 36);
    const line = (m.role === 'assistant' ? 'Hermes: ' : '> ') + text;
    if (full) { newestFirst.push(line); budget = Math.max(0, budget - line.length); continue; } // 最新一轮完整(优先)
    if (budget - line.length < 0) continue; // 折叠消息:预算满则跳过(更旧同理)
...
export function mainContent(state: State, tickIndex = 0): string {
  switch (state.kind) {
    ...
    case 'idle':
      if (state.loading) return 'Loading ' + SPINNER_FRAMES[tickIndex % SPINNER_FRAMES.length];
      return historyText(state.history, ''); // 无历史时空白(不显示操作指南)
    case 'recording':
      return historyText(state.history, ''); // 历史页内录音:显示历史,顶部提示 listening
    case 'transcribing':
    case 'thinking':
      return historyText(state.history, '...');
    case 'displaying': {
